...
template <typename T>
inline T hann(T p) {
	return T(0.5) * (1 - CTAG::SP::HELPERS::fastcos(2 * T(M_PI) * p));
}
...
inline T blackman(T alpha, T p) {
	return
	  + (1 - alpha) / 2
	  - T(1) / 2 * CTAG::SP::HELPERS::fastcos(2 * T(M_PI) * p)
	  + alpha / 2 * CTAG::SP::HELPERS::fastcos(4 * T(M_PI) * p);
}
...
inline T blackmanNuttall(T p) {
	return
	  + T(0.3635819)
	  - T(0.4891775) * CTAG::SP::HELPERS::fastcos(2 * T(M_PI) * p)
	  + T(0.1365995) * CTAG::SP::HELPERS::fastcos(4 * T(M_PI) * p)
	  - T(0.0106411) * CTAG::SP::HELPERS::fastcos(6 * T(M_PI) * p);
}
...
inline T blackmanHarris(T p) {
	return
	  + T(0.35875)
	  - T(0.48829) * CTAG::SP::HELPERS::fastcos(2 * T(M_PI) * p)
	  + T(0.14128) * CTAG::SP::HELPERS::fastcos(4 * T(M_PI) * p)
	  - T(0.01168) * CTAG::SP::HELPERS::fastcos(6 * T(M_PI) * p);
}
