...
void minBlepImpulse(int z, int o, float* output) {
	...
	rfft.rfft(x, fx);
	// fx = exp(fx)
	fx[0] = CTAG::SP::HELPERS::fastexp(fx[0]);
	for (int i = 1; i < n; i++) {
		float re = CTAG::SP::HELPERS::fastexp(fx[2 * i]);
		float im = fx[2 * i + 1];
		fx[2 * i] = re * CTAG::SP::HELPERS::fastcos(im);
		fx[2 * i + 1] = re * CTAG::SP::HELPERS::fastsin(im);
	}
	fx[1] = CTAG::SP::HELPERS::fastexp(fx[1]);
	rfft.irfft(fx, x);
	rfft.scale(x);
